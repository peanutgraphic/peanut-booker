...
if (!class_exists('Peanut_License_Client')) {
    ...
    class Peanut_License_Client {
        ...
        public function check_for_updates($transient) {
            ...
            if (!empty($response['update_available']) && !empty($response['plugin_info'])) {
                ...
                $transient->response[$plugin_file] = (object) [
                    ...
                    'slug' => $info['slug'] ?? $this->config['plugin_slug'],
                    'plugin' => $plugin_file,
                    'new_version' => $info['new_version'] ?? $this->config['version'],
                    'url' => $info['homepage'] ?? '',
        ...
        public function plugin_info($result, $action, $args) {
            ...
            return (object) [
                'name' => $response['name'] ?? $this->config['plugin_name'] ?? '',
                'slug' => $response['slug'] ?? $this->config['plugin_slug'],
                'version' => $response['version'] ?? $this->config['version'],
        ...
        /**
         * Report site health to license server.
         * Called automatically once per day via cron.
         */
        public function report_health(): array {
            $license_key = $this->get_license_key();
            ...
            if (empty($license_key)) {
                return [
                    'success' => false,
                    'error' => 'no_license',
                    'message' => __('No license key to report health for.', 'peanut-license-client'),
                ];
            }
            ...
            if (version_compare(PHP_VERSION, '8.0', '<')) {
                $errors[] = 'PHP version is below recommended 8.0';
            }

            // Check WordPress version.
            if (version_compare($wp_version, '6.0', '<')) {
                $errors[] = 'WordPress version is below recommended 6.0';
            }

            // Check if debug mode is enabled on production.
            if (defined('WP_DEBUG') && WP_DEBUG && !defined('WP_DEBUG_LOG')) {
                $errors[] = 'Debug mode enabled without logging';
            }
            ...
            $response = $this->api_request('site/health', $health_data, 'POST');

            // Schedule next report.
            if ($response['success'] && !empty($response['next_report'])) {
                update_option($this->config['status_option'] . '_next_health', $response['next_report']);
            }
        ...
        /**
         * Get activation usage info.
         */
        public function get_activation_info(): array {
            $license_key = $this->get_license_key();
            ...
            return $this->api_request('license/activations', [
                'license_key' => $license_key,
            ], 'GET');
        }

        /**
         * Check if at activation limit.
         */
        public function is_at_activation_limit(): bool {
            $license_data = $this->get_license_data();
            ...
            $used = $license_data['activations_used'] ?? 0;
            $limit = $license_data['activations_limit'] ?? 1;
        ...
        public function get_upgrade_url(): string {
            return 'https://peanutgraphic.com/' . $this->config['plugin_slug'] . '/pricing/';
        }

        /**
         * Render activation limit notice.
         */
        public function render_activation_limit_notice(): void {
            $license_data = $this->get_license_data();
            $used = $license_data['activations_used'] ?? 0;
            $limit = $license_data['activations_limit'] ?? 1;
            ...
            ?>
            <div class="notice notice-warning peanut-activation-limit-notice" style="padding: 12px; border-left-color: #f59e0b;">
                <h3 style="margin: 0 0 8px;"><?php esc_html_e('Activation Limit Reached', 'peanut-license-client'); ?></h3>
                <p style="margin: 0 0 12px;">
                    <?php printf(
                        esc_html__('Your %s license is activated on %d of %d allowed sites. To activate on more sites, please upgrade your license.', 'peanut-license-client'),
                ...
                <p style="margin: 0;">
                    <a href="<?php echo esc_url($this->get_upgrade_url()); ?>" class="button button-primary" target="_blank">
                        <?php esc_html_e('Upgrade License', 'peanut-license-client'); ?>
                    </a>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=' . $this->config['plugin_slug'] . '&tab=license')); ?>" class="button" style="margin-left: 8px;">
                        <?php esc_html_e('Manage Activations', 'peanut-license-client'); ?>
                    </a>
                </p>
            </div>
            <?php
        ...
        /**
         * Render license status widget for dashboard.
         */
        public function render_status_widget(): void {
            ...
            $status_color = match($status) {
                'active' => '#22c55e',
                'expired' => '#f59e0b',
                'invalid', 'suspended', 'revoked' => '#ef4444',
                default => '#6b7280',
            };

            ?>
            <div class="peanut-license-widget" style="padding: 15px; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background: <?php echo esc_attr($status_color); ?>;"></span>
                    <strong style="font-size: 14px;"><?php echo esc_html($this->config['plugin_name']); ?></strong>
                </div>

                <?php if ($status === 'active' && $license_data) : ?>
                    <div style="font-size: 13px; color: #6b7280;">
                        <div style="margin-bottom: 4px;">
                            <span style="color: #111827; font-weight: 500;"><?php echo esc_html(ucfirst($license_data['tier'] ?? 'free')); ?></span>
                            <?php esc_html_e('License', 'peanut-license-client'); ?>
                        </div>
                        <div>
                            <?php printf(
                                esc_html__('Sites: %d / %d', 'peanut-license-client'),
                ...
                <?php else : ?>
                    <div style="font-size: 13px; color: #6b7280;">
                        <?php esc_html_e('License not activated', 'peanut-license-client'); ?>
                    </div>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=pb-settings&tab=license')); ?>" class="button button-small" style="margin-top: 8px;">
                        <?php esc_html_e('Activate License', 'peanut-license-client'); ?>
                    </a>
                <?php endif; ?>
            </div>
            <?php
         ...
         */
        public function schedule_health_report(): void {
            if (!wp_next_scheduled('peanut_license_health_report_' . $this->config['plugin_slug'])) {
                wp_schedule_event(time() + DAY_IN_SECONDS, 'daily', 'peanut_license_health_report_' . $this->config['plugin_slug']);
            }
        }

        /**
         * Unschedule health report.
         */
        public function unschedule_health_report(): void {
            wp_clear_scheduled_hook('peanut_license_health_report_' . $this->config['plugin_slug']);
        }
    }
}
